...
class RechercheController extends Controller
{
    ...
    public function afficheRechercheParMotCleAction(Request $request, LoggerInterface $logger)
    {
        $this->cat = "Article";
        $this->initAmazon($this->cat);
        /*$recherche = $request->request->get("search_bar", "Booba");*/
        ...
        $query->select('a');
        $query->from('AppBundle\Entity\Catalogue\Article', 'a');
        $query->where('a.titre like :motCle');
        ...
        /*$query = $this->entityManager->createQuery("SELECT a FROM AppBundle\Entity\Catalogue\Article a WHERE a.titre like '%".addslashes("plk")."%'");*/
    ...
    public function initAmazon($cat) {
        if (((count($this->entityManager->getRepository("AppBundle\Entity\Catalogue\Musique")->findAll()) == 0) && $cat == "Music" ) || ((count($this->entityManager->getRepository("AppBundle\Entity\Catalogue\Livre")->findAll()) == 0) && $cat == "Books" ) || ((count($this->entityManager->getRepository("AppBundle\Entity\Catalogue\Article")->findAll()) == 0) && $cat == "Article" )) {
